...
# Workspace sandbox root — same boundary used by filesystem.py
...
def execute(directory: str, max_files: int = 10, recursive: bool = False) -> str:
    ...
    try:
        ...
        # Sandbox enforcement
        if not str(target_path).lower().startswith(str(_SANDBOX).lower()):
            return f"Error: Access denied. '{directory}' is outside the allowed workspace sandbox."
        ...
        report.append(f"Folders ({len(folders)}): {', '.join(str(f) for f in sorted(folders)[:20])}")
        
        file_entries = []
        for f in sorted(files)[:50]: # Cap listing to avoid overflow
            full_path = target_path / f
            mtime = os.path.getmtime(full_path)
            mtime_str = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(mtime))
            file_entries.append(f"{f} (Modified: {mtime_str})")
