...
## v1.8.0 — (2026-05-01) Phase G: Legacy Retirement & Cold-Start Replacement
**Loop + Shim Deletion, Four-Cognate GUI, env_audit Similarity Cold-Start**
### Key Improvements
*   **`core/loop.py` and `core/lx_loop_shim.py` deleted & CONTEXTUALIZE merged (ADR D-20260428-01)**: The legacy six-step CoreLoop and its monkey-patch shim are gone from the repo. Both files had been structurally dormant since Phase F. The `CONTEXTUALIZE` step's responsibilities—querying `lx_memory.db` for `active_tasks` and `working_memory`, and fetching `recent_turns`—were successfully absorbed into the end of `lx_Observe.execute`. This formally reduces the Servo Core down to a dense 4-cognate sequence. `ServoCore.run_cycle`'s former shim install/uninstall scaffolding is also gone — the `_lx_shim_handle` defensive read is retired, the `_active_store` attribute remains as the cognate-only handle for stash/clear bookkeeping. `gui/loop_panel.py`'s `from core.lx_steps import LoopStep` import path stays (Phase E's re-export module) so the GUI doesn't reach into a deleted file.
*   **Four-cognate GUI rewrite**: `gui/loop_panel.py` rewritten to render the four-cognate Servo Core sequence (`OBSERVE` / `REASON` / `ACT` / `INTEGRATE`) with active-step highlight driven by `current_step` from the cognate state. The INSTALLED TOOLS dropdown was restyled in `gui/tool_panel.py` to match the Stream Viewer / Log Viewer idiom — collapsible header + dropdown content, no more always-visible enumerated rows. The trace section below the loop-state panel was removed; `gui/main_window.py` no longer wires `trace_event` to a sink. The `trace_event` signal itself stays defined on `ServoCoreThread` for any future telemetry consumer — emits land on no listener and Qt swallows them cleanly.
*   **env_audit similarity cold-start (ADR D-20260428-01)**: `lx_StateStore` gained a second ChromaDB collection — `env_snapshots` under `state/chroma_env_snapshots_<profile>/` (cosine metric, mirrors `procedural_wins` shape). `commit_env_snapshot(env_audit, embedding, tool_name, reward)` writes one row per successful cycle whose reward clears the commit threshold; the row carries the env_audit dict as the document, the audit embedding as the vector, and `(tool_name, reward, timestamp, embed_source, embed_dim)` in metadata. `query_env_snapshots(obs_embedding, limit, similarity_floor)` performs cosine kNN against the audit fingerprint with the `env_snapshot_similarity_floor` config key (default `0.6` — intentionally looser than `nn_similarity_floor`'s `0.7` because env_snapshots index the audit fingerprint rather than the prose observation signature). `count_env_snapshots()` exposes the collection size. Stable IDs are derived from `sha1(audit_blob | tool_name | reward)[:16]` so an idempotent re-commit collapses rather than ballooning the index, and the JSON encoding uses `sort_keys=True` so structurally-identical audits fingerprint the same regardless of dict iteration order.
*   **`lx_Reason._exploit` rewired as a two-tier cascade**: On a `procedural_wins` miss, REASON queries `env_snapshots` via `store.query_env_snapshots(obs_embedding=obs_emb)`. Each returned neighbor is scored as `reward × similarity` and deduped by `tool_name` with `max`. When the prior cycle had a stderr flag, the last-failing tool is excluded from the candidate set. The highest-scoring tool is exploited; on a double-miss (procedural_wins empty AND env_snapshots returns nothing above floor), `_exploit` falls through to neutral ε-greedy `_explore()`. The Phase E `_observation_signal` classifier (and its `_sum_total_symbols` / `_drift_percent` helpers) and the `_COLD_START_LOOKUP` table were deleted — the procedural-recall layer is now entirely vector-driven, with the only hard-coded floor being the similarity threshold.
*   **`lx_Integrate.execute` extended to commit env_snapshots**: alongside the existing `commit_success_vector` call, INTEGRATE now calls `store.commit_env_snapshot(env_audit, observation_embedding, tool_name, reward)` on every cycle whose reward clears the threshold and records `last_env_committed: True/False` in the delta. Both commit calls are wrapped in try/except so a chromadb failure on either path doesn't break the cognate loop; the trace string was updated to surface env-commit telemetry alongside the existing procedural-commit signal.
*   **ServoCoreThread wiring hotfix (ADR D-20260427-01)**: Phase G boot triage caught a Phase F miss where `ServoCoreThread.__init__` was passing the legacy `core.state.StateStore` into `ServoCore.run_cycle` instead of constructing an `lx_StateStore`. Fixed: `ServoCoreThread.__init__` now constructs `lx_StateStore(profile=..., state_dir=..., legacy_config_path=..., config=core.config)` and passes that into `core.run_cycle(store)`. The legacy `StateStore` wiring stays in place for the GUI's chat-history reload path (which reads `state.db` directly), but the cognate dispatch surface gets the Sovereign Ledger explicitly.
*   **lx_correctness handshake test updated (ADR D-20260427-02)**: `benchmark/criteria/lx_correctness.py::test_core_loop_handshake` was hanging indefinitely under Phase F's park/wake gate because OBSERVE waited on `perception_cond` with no producer ever submitting a perception. Fixed: the test now feeds a synthetic `user_input` perception via `core.submit_perception(...)` before `run_cycle`, and halts on the OBSERVE→REASON transition (rather than the original REASON→ACT) so the assertion runs at the first cognate boundary the perception traverses — sidestepping both the chooser-policy non-determinism and Phase G's env_snapshots cold-start bootstrap window.

### Acceptance
*   **Phase A audit green**: `python -m benchmark.lx_audit_manager` exits 0 with `Overall Pass: True` (`audit_20260501_050324.json`). All four `lx_correctness` tests pass; `lx_lexicon` and `lx_performance` audits clean.
*   **Structural verification**: `core/loop.py` and `core/lx_loop_shim.py` absent from the repo. `lx_StateStore` exposes `commit_env_snapshot` / `query_env_snapshots` / `count_env_snapshots`. `lx_Reason._observation_signal` / `_sum_total_symbols` / `_drift_percent` and `_COLD_START_LOOKUP` are absent from `core/lx_cognates.py`. `core/config_registry.py` `_DEFAULTS` carries `env_snapshot_similarity_floor: 0.6`. `gui/loop_panel.py` renders the four-cognate sequence; `gui/tool_panel.py` uses the dropdown idiom; `gui/main_window.py` no longer wires `trace_event`. `codex/manifest.json` and `codex/manifest_compact.json` carry `"version": "1.8.0"`.
*   **Smoke test**: `lx_StateStore.commit_env_snapshot(...)` round-trip on a fresh profile commits one row, `query_env_snapshots(obs_embedding=same_vec)` retrieves it with `_similarity ~= 1.0`, `reset()` clears `current_state` to `{"current_step": "OBSERVE", "last_trace": None}`. Compile-clean across `core/lx_state.py`, `core/lx_cognates.py`, `core/config_registry.py`, `core/core.py`.

### Internals
*   `_env_snapshots = None` graceful-degradation pattern matches `_procedural_wins`: if chromadb fails to import or the index is corrupt, both commit and query paths short-circuit cleanly without breaking the cognate loop.
*   The two ChromaDB collections live in independent `PersistentClient` instances (`_chroma_client` rooted at `chroma_procedural_wins_<profile>/`, `_env_chroma_client` rooted at `chroma_env_snapshots_<profile>/`) because chromadb's `PersistentClient` is path-bound — one client per on-disk index. An env_snapshots failure cannot take procedural_wins down, and vice versa.
*   `query_env_snapshots` has no exact-match fallback (unlike `query_success_vectors`) because env_snapshots is keyed on the audit fingerprint, not a human-readable signature — a string-match branch wouldn't be meaningful. A degenerate query embedding just returns `[]`.
*   `_exploit`'s last-failing-tool exclusion fires only when `state.get("stderr_flag")` is truthy AND the excluded tool is in `env_best`; a procedural_wins-only path keeps its existing dedup logic. The cascade order is: procedural_wins exact-match → procedural_wins semantic kNN → env_snapshots semantic kNN → ε-greedy `_explore()`.
*   `lx_Integrate`'s commit ordering is procedural_wins first, env_snapshots second; if procedural_wins raises, env_snapshots still attempts (independent try/except). The delta surface carries `last_committed: bool` (procedural) and `last_env_committed: bool` (env_snapshot) so a Phase H+ telemetry consumer can observe each commit channel independently.
...
## v1.7.0 — (2026-04-26) Phase F: Chat-as-Perception & Cognate-Native Dispatch
**Perception Queue, Park/Wake Gate, Response Hook, Hot-Reload, Toggle Retirement**

### ✨ Key Improvements
*   **Chat-as-perception architecture (ADR D-20260426-01)**: User input no longer enters via a side-channel `submit_input` queue feeding a CoreLoop branch. It enters as an `lx_Observe` perception event, partitioned by an `observation_kind` field (`"user_input"` | `"tool_output"`). OBSERVE is the single ingress; REASON sees the same delta surface regardless of source. The Phase E "Phase F" acknowledgement message is retired — `submit_input` now genuinely drives a cognate cycle.
*   **Perception queue + park/wake gate on `ServoCore`**: `ServoCore.__init__` now owns `perception_queue` (a `collections.deque`), `perception_cond` (a `threading.Condition`), and `halt_event` (a `threading.Event`). `submit_perception(event)` validates shape, stamps a `timestamp` if missing, appends, and `notify_all`s under the condition. `signal_halt()` sets the event and notifies, so a Stop button click breaks the park without needing a sentinel enqueue. `lx_Observe.execute` was rewritten as a park/wake gate: it consults `state["pending_tool_output"]` first (cross-cycle handoff from ACT); if absent, it acquires `perception_cond` and `wait`s with a 0.5s halt-aware timeout, popping from the queue when one arrives. Headless callers (benchmarks, unit tests) get the same surface — Qt is not in the loop.
*   **Cross-cycle ACT→OBSERVE handoff via `pending_tool_output`**: `lx_Act._wrap_delta` writes `delta["pending_tool_output"] = {"kind": "tool_output", "tool_result": <ToolOutcome>, ...}` after a successful tool dispatch unless the cycle is halting or the dispatch resolved to `<none>` (`outcome.status == "skip"`). The next OBSERVE consumes it and emits a `[TOOL_OUTPUT]` perception block; REASON then chooses a follow-up tool or a `<none>` to surface the result. The slot is cleared on every OBSERVE delta so a stale handle can't leak into a later cycle.
*   **`response_ready_hook` on `ServoCore` + `record_turn` persistence**: `lx_Integrate.execute` calls `core.response_ready_hook(text, image_b64="")` whenever REASON produced prose, and `_persist_turn(...)` writes the turn to a dedicated `state/lx_turns_<profile>.db` SQLite store via `lx_StateStore.record_turn`. `ServoCoreThread.__init__` binds the hook to its `response_ready` Qt signal so chat output flows through the cognate path the same way it did from CoreLoop, but headless callers leave the hook `None` and the loop runs silently. `query_turns(limit=50)` reads back the persisted history; the schema is `(id, perception_text, observation_kind, response_text, tool_name, status, timestamp)` with an index on `timestamp`.
*   **`ToolContext` dataclass + per-dispatch context injection**: `lx_Act._build_tool_context()` constructs a `ToolContext(state, config, telemetry, conn_factory, ollama, legacy_loop_ref)` and injects it into the five `_CONTEXT_TOOLS` (`memory_manager`, `memory_snapshot`, `task`, `context_dump`, `system_config`) as a keyword-only `tool_context` kwarg. `legacy_loop_ref` carries an `LxLoopAdapter` so `system_config` / `context_dump`'s `_get_loop_ref()` reads still resolve, but the adapter is now a per-dispatch value rather than a module-level monkey-patch. The `conn_factory` kwarg from Phase E is preserved alongside `tool_context` for back-compat with tools whose tests pass it directly.
*   **`_get_loop_ref` shim retired (`core/lx_loop_shim.py` install path)**: `ServoCore.run_cycle` no longer installs `_Sqlite3Proxy` or rewrites `_get_loop_ref`. Both surfaces are now per-dispatch injections via `ToolContext`. The shim module remains on disk as a dormant rollback target — every monkey-patch entry point in the cognate dispatch path is now gone. `_lx_shim_handle` is kept set to `None` so any code that defensively reads it continues to see the no-shim signal.
*   **mtime-driven hot-reload on `ConfigRegistry`**: `ConfigRegistry.maybe_reload()` stats `codex/manifests/config.json` on every cycle (one `os.stat` call when unchanged), compares `st_mtime_ns` against a cached value, and re-reads the overlay only when the file changed. Malformed reloads update the mtime cache anyway so the next cycle doesn't burn CPU retrying a poison file. `ServoCore.run_cycle` calls `maybe_reload()` at the top of each `while True` iteration; in-flight cognate frames keep the value they read at frame start, which avoids the mid-cycle mismatch a snapshot-by-cycle policy is designed to prevent.
*   **`USE_SERVO_CORE` toggle retired (v1.7.0 release gate)**: `gui/main_window.py` no longer reads `SERVO_CORE` env or `use_servo_core` config. The constructor unconditionally builds `ServoCoreThread`. The `from core.loop import CoreLoop` import is commented out as a retirement note; `core/loop.py` itself remains under No-Write discipline. `gui/tool_panel.py`'s `on_tool_dispatched` listener docstring updated to note ServoCoreThread is now the only construction path. CoreLoop's eventual deletion is a Phase G concern; keeping it dormant satisfies the release-gate rule that only one production path exists.

### 🧪 Acceptance
*   **Phase A audit (deferred to local execution)**: `python -m benchmark.lx_audit_manager` to be run on Kevin's machine due to a OneDrive sync lag that consistently serves bash a stale truncated view of `core/lx_cognates.py`. The Read/Edit tool path saw the full file at every step; the ~1156-line module landed correctly on disk. Phase A green/red is the v1.7.0 gate-of-record.
*   **Structural verification**: `git diff --ignore-cr-at-eol core/loop.py` empty (No-Write satisfied). `gui/main_window.py` no longer references `USE_SERVO_CORE`, `SERVO_CORE`, `_resolve_use_servo_core`, or `from core.loop import CoreLoop`. `core/lx_loop_shim.py` install/uninstall calls absent from `ServoCore.run_cycle`. `lx_Observe.execute` is a park/wake gate; `lx_Act._wrap_delta` writes `pending_tool_output`; `lx_Integrate.execute` fires `response_ready_hook` and calls `record_turn`. `core/config_registry.py` exposes `maybe_reload()`. `codex/manifest.json` and `codex/manifest_compact.json` carry `"version": "1.7.0"`.

### 🔧 Internals
*   `_CONTEXT_TOOLS = _MEMORY_TOOLS | frozenset({"system_config"})` is the authoritative Phase F dispatch surface for `tool_context` injection. Tools outside this set receive the legacy positional/keyword call shape unchanged.
*   `submit_perception` is single-producer-single-consumer in the common case (GUI submits, OBSERVE consumes); `notify_all` is used so future fanout (mirror, telemetry tap) wouldn't need a code change here.
*   `record_turn` opens a fresh `sqlite3` connection per call with `PRAGMA journal_mode=WAL` so concurrent reads from a Phase G+ history viewer don't block writes.
*   `_fire_response_ready` swallows hook exceptions defensively — a misbehaving GUI listener must not crash the cognate loop.
*   `LxLoopAdapter` is constructed once per ACT dispatch (not memoized across cycles) so changes to `state` / `config` / `telemetry` between cycles are picked up cleanly.

### 🧭 Phase G Handoff
*   Delete `core/loop.py` and `core/lx_loop_shim.py` (full retirement, lifting No-Write).
*   Add a `query_turns` UI surface so the chat history is browsable from the GUI.
*   Telemetry-driven tuning loop on top of `ConfigRegistry.maybe_reload()` (e.g. lambda decay schedule reacts to drift signal volume).
*   Multi-listener `response_ready_hook` fanout if a Servo mirror/observer pane lands.
*   Replace the ACT-writes-pending-tool-output convention with an explicit cross-cycle event bus once a third producer (background timer? scheduled task?) needs to inject perception.

---

## v1.6.0 — (2026-04-25) Phase E: Configuration Migration & GUI Rewire
**ConfigRegistry, ServoCoreThread, Cold-Start Enrichment**

### ✨ Key Improvements
*   **ConfigRegistry as single source of truth (`core/config_registry.py`)**: Seven Phase D literals (`epsilon_0`, `lambda_decay`, `embed_dim`, `commit_threshold`, `nn_similarity_floor`, `embed_model`, `observe_roots`) migrated into `codex/manifests/config.json` with `_DEFAULTS` as the safety net. `ConfigRegistry()` reads once at instantiation; missing/malformed config files degrade silently to defaults rather than raising. Overlay-not-replace semantics on `reload()`.
*   **Config threaded through ServoCore + Cognates + lx_StateStore**: `ServoCore.__init__(config=None)` instantiates a default registry; cognates resolve tunables via `self.core.config.get(key, literal_fallback)` so the literal stays the safety net when a registry is absent. `OllamaClient` keeps `embed_model="nomic-embed-text"` as its declared default (Q2 — literal-as-default), with `ConfigRegistry.get` overriding only when a registry is passed. Phase A audit green; no behavioral change vs. Phase D defaults.
*   **`conn_factory` kwarg as a uniform tool-context contract**: `memory_manager`, `memory_snapshot`, `task`, and `context_dump` all accept a keyword-only `conn_factory` kwarg (Q3 — uniform extension). Default path unchanged: when omitted, tools fall back to the legacy `sqlite3.connect("state/state.db")` so standalone tool tests still pass. Establishes the surface Phase F's broader context-injection contract will plug into.
*   **Memory-routing shim retired**: `ServoCore.run_cycle` now injects `conn_factory=self._lx_conn_factory` at dispatch, replacing the global `sqlite3.connect` monkey-patch in the memory branch of `core/lx_loop_shim.py`. `LxLoopAdapter` (for `system_config` / `context_dump`'s `_get_loop_ref()` reads) stays untouched — refactoring those tools is a Phase F concern. `sha256(state/state.db)` byte-identical before/after a 100-cycle bench under the conn_factory path.
*   **Cold-start signal enrichment**: `ServoCore._prior_audit_snapshot` lives across run_cycle invocations within a single ServoCore lifetime; `lx_Integrate` writes the cycle's env_audit to it at the end of every successful pass so the next REASON has something to diff against. `lx_Reason._observation_signal(state)` rewritten to classify into three branches: `empty_project` (env_audit has no roots), `drift_detected` (≥20% delta in any of five symbol-tuple dimensions vs. prior snapshot), `directory_scan` (otherwise — first cycle or no drift). `_COLD_START_LOOKUP` expanded with branch-specific exploit weights. The `manifest_stale` heuristic dropped from scope (Q6 — too noisy on a markdown-heavy codex).
*   **GUI rewire behind `USE_SERVO_CORE` toggle**: `gui/main_window.py` now constructs `ServoCoreThread` (a QThread adapter wrapping `ServoCore`) by default. Toggle resolves from `SERVO_CORE` env var first, then `use_servo_core` in `codex/manifests/config.json`, defaulting to True (Q4). Rollback to `CoreLoop` is a single env flip (`SERVO_CORE=0`). New `core/lx_servo_thread.py` (227 lines) mirrors `CoreLoop`'s 12-signal Qt surface 1:1 and adds a `tool_dispatched(str)` signal so `gui/tool_panel.py` can highlight the active atomic primitive. `lx_Act` fires `_lx_dispatch_hook` right before invoking the registry; the wrapper translates that into a Qt signal emit. `core/lx_steps.py` (24 lines) re-exports `LoopStep` so `gui/loop_panel.py` doesn't reach into `core.loop` (No-Write discipline).
*   **`ATOMIC_PRIMITIVES` exposed on ServoCore**: Public frozenset at class scope (Q5 — single-import directness over method-wrapping). `gui/tool_panel.py` reads it once at module load and renders an `[A]` badge on each of the eleven primitive rows. Iteration order is not part of the contract.

### 🧪 Acceptance
*   **§9 acceptance gates**: All eight Phase E gates pass. `python -m benchmark.lx_audit_manager` exits 0 with `Overall Pass: True` (`audit_20260425_155609.json`). ConfigRegistry round-trip (write override → 0.5; delete file → 0.3 default). `git diff --ignore-cr-at-eol core/loop.py` empty (No-Write satisfied; only line-ending churn shows under default diff). `_observation_signal` returns `empty_project` / `directory_scan` / `drift_detected` correctly on synthetic states. GUI smoke under `SERVO_CORE=1` constructs `ServoCoreThread` and boots; under `SERVO_CORE=0` constructs `CoreLoop` and boots. `loop_panel` renders the six Cognate phases (`PERCEIVE`/`CONTEXTUALIZE`/`REASON`/`ACT`/`INTEGRATE`/`OBSERVE`) in Phase D order. `lx_StateStore.commit_success_vector` accepts new entries — `procedural_wins` schema preserved. No new file >300 lines (`config_registry.py`=152, `lx_steps.py`=24, `lx_servo_thread.py`=227).
*   **Step 5 synthetic-state suite**: 14 assertions covering all three classifier branches plus helpers and `lx_Integrate` cache write. ConfigRegistry tests: 14/14.

### 🔧 Internals
*   `_PROJECT_ROOT` resolved once at `core/config_registry.py` import; `_DEFAULTS` is the class-level dict, copied (not aliased) into the per-instance overlay so `reload()` mutations never leak into the class.
*   `ServoCoreThread` instantiates `ServoCore` directly (no Qt inheritance on the cognate side — keeps the headless benchmark path PySide6-free) and supervises `run_cycle` inside a watchdog `apply_delta` wrapper so `stop()` propagates a `halt=True` injection on the next cycle.
*   `submit_input` on the Servo path returns an honest "Phase F" acknowledgement so the GUI doesn't deadlock — chat-driven cognate dispatch is Phase F's scope.
*   The `_get_loop_ref` adapter for `system_config` / `context_dump` survives Phase E because those tools read more than a connection (they read `loop.state`, `loop.config`, `loop.telemetry`) — refactoring requires the broader Phase F context-injection contract.

### 🧭 Phase F Handoff
*   Replace `_get_loop_ref` with a uniform context-injection contract (the natural extension of Phase E's `conn_factory` kwarg).
*   Wire `submit_input` into a real cognate-driven chat dispatch path; retire the Phase E acknowledgement message.
*   Consider replacing the cold-start heuristic with a similarity query against env_audit snapshots in ChromaDB (reusing Phase D NN infrastructure).
*   Drop the `USE_SERVO_CORE` toggle once the rewire has accumulated a couple of real-world cycles.
*   Hot-reload (mtime-driven) on `ConfigRegistry` and telemetry-driven tuning on top of the registry surface.
